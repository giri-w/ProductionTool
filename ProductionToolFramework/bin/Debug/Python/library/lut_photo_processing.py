...
def lut_photo_processing(hand, data4_path, data_path, dsp_out, thrIntensity, thrMass, thrDistance):
  ...
  ## compose mask grid4
   ...
   if dsp_out and picture:
      display_grid(grid4, photo4, i+1,hand)
   ...
   if dsp_out and picture: 
      display_grid(grid, photo, i+2,hand)
   ...
   ## detect photo grid step1
   
   grid  = detect_photo_grid(grid, photo, thrIntensity, thrMass)                
   
   if dsp_out and picture:
      display_grid(grid, photo, i+3,hand)
   ...
   ## extrapolate photo step1
   
   grid  = extrapolate_photo_grid(grid)                                         
   
   if dsp_out and picture:
      display_grid(grid, photo, i+4,hand)
   ...
   ## detect photo grid step2
   
   grid  = detect_photo_grid(grid, photo, thrIntensity, thrMass)                
   
   if dsp_out and picture:
      display_grid(grid, photo, i+5,hand)
   ...
   ## extrapolate photo step2
   
   grid  = extrapolate_photo_grid(grid)                                         
   
   if dsp_out and picture :
      display_grid(grid, photo, i+6,hand)
   ...
   ## detect photo grid step3   
  
   grid  = detect_photo_grid(grid, photo, thrIntensity, thrMass)                
   
   if dsp_out and picture:
      display_grid(grid, photo, i+7,hand)
   ...
   ## extrapolate photo step3
   
   grid  = extrapolate_photo_grid(grid)                                         
   
   if dsp_out and picture :
      display_grid(grid, photo, i+8,hand)
   ...
   if hand == 'left':
      ...
      if dsp_out and picture:
         display_grid(grid, photo, i+9,hand)
...
def display_grid(grid, photo_filename, fignr,hand):
   ...
   for k in range (0,grid_size[0]):
      for n in range (0,grid_size[1]):
         if not(grid[k][n].status == 'initial'):
            ...
            x_lim = np.array(x_lim,dtype='int')
            
            x_dec = np.amin([[512-1,640-1],np.amax([[1,1],x-1],axis=0)],axis=0)
            x_dec = np.array(x_dec,dtype='int')
            
            x_inc = np.amin([[512-1,640-1],np.amax([[1,1],x+1],axis=0)],axis=0)
            x_inc = np.array(x_inc,dtype='int')
            
          
            
            # give the color to the pixel
            for q in range (0,3):
               photo[x_dec[1]][x_lim[0]][q] = color[q]
               photo[x_inc[1]][x_lim[0]][q] = color[q]
               photo[x_lim[1]][x_lim[0]][q] = color[q]
               photo[x_lim[1]][x_dec[0]][q] = color[q]
               photo[x_lim[1]][x_inc[0]][q] = color[q]
